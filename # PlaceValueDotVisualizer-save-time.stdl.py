# PlaceValueDotVisualizer-save-time-halfheight.py
...
def draw_number(number_str):
    ...
    box_side = 7 * dot_diameter  # Box is a square: height = width = 7 diameters
    fig, ax = plt.subplots(figsize=(10, box_side))

    ax.set_xlim(0, 4 * box_side)
    ax.set_ylim(0, box_side)

    ax.add_patch(
        patches.Rectangle(
            (0, 0), 4 * box_side, box_side, linewidth=2, edgecolor='green', facecolor='lightgreen'
        )
    )

    for i in range(1, 4):
        x = i * box_side
        ax.plot([x, x], [0, box_side], color='green', linewidth=2)

    ax.axis('off')

    labels = ['Thousands', 'Hundreds', 'Tens', 'Ones']
    for i in range(4):
        ax.text((i + 0.5) * box_side, box_side + dot_diameter * 0.3, labels[i], ha='center', va='bottom', fontsize=7)

    number_str = number_str.zfill(4)
    cols = 3
    rows = 3
    spacing = (box_side - (cols * dot_diameter)) / (cols + 1)
    padding = spacing  # equal spacing from edges
    ...
    for idx, digit_char in enumerate(number_str):
        ...
        start_x = idx * box_side + padding + dot_radius
...
def main():
    ...
    student_name = st.text_input("Enter your name or ID:", "student")
    ...
    if st.button("Save Image"):
        if number_input.isdigit() and len(number_input) <= 4:
            ...
            folder = student_name.strip().replace(" ", "_") or "student"
            os.makedirs(folder, exist_ok=True)
            full_filename = os.path.join(folder, f"{filename}_{timestamp}.jpg")
